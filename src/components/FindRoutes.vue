  ...
  <div class="container">
    <div v-if="routeData" class="route-card">
      <div class="route-header">
        <h2>
          <span class="start">📍 {{ routeData.startLocation }}</span>
          <span class="arrow">→</span>
          <span class="end">🏁 {{ routeData.endLocation }}</span>
        </h2>
      ...
      <div class="comments-section">
        ...
        <div class="comment-list" v-if="commentData && commentData.length">
          <div class="comment-item" v-for="(c, index) in commentData" :key="index">
            <div class="comment-avatar">👤</div>
            <div class="comment-content">
              <div class="comment-meta">
                <span class="username">Пользователь #{{ userData[c.userId]?.username || 'Неизвестный' }}</span>
                <span class="timestamp">{{ new Date(c.createAt).toLocaleString() }}</span>
              </div>
              <p>{{ c.comment }}</p>
            </div>
          </div>
